...
function Mark({ size=32 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" style={{display:"block"}}>
      ...
      <g transform="translate(8,8) rotate(-27)"><polygon points="0,-6 -3.5,3.5 3.5,3.5" fill={C.green} /></g>
      <path d="M 32,52 C 38,40 48,24 56,8" stroke={C.amber} strokeWidth="3.8" strokeLinecap="round" fill="none" />
      <g transform="translate(56,8) rotate(27)"><polygon points="0,-6 -3.5,3.5 3.5,3.5" fill={C.amber} /></g>
...
const NAV = [
  ...
  { id:"trade",        path: "/dashboard/trade",  label:"New Trade",            icon:<svg width={15} height={15} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg> },
  { id:"withdraw",     path: "/dashboard/withdraw",label:"Withdraw",            icon:<svg width={15} height={15} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/><line x1="5" y1="19" x2="19" y2="19"/></svg> },
  ...
  { id:"bank",         path: "/dashboard/bank",   label:"Bank Accounts",        icon:<svg width={15} height={15} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round"><line x1="3" y1="22" x2="21" y2="22"/><line x1="6" y1="18" x2="6" y2="11"/><line x1="10" y1="18" x2="10" y2="11"/><line x1="14" y1="18" x2="14" y2="11"/><line x1="18" y1="18" x2="18" y2="11"/><polygon points="12 2 20 7 4 7"/></svg> },
  { id:"kyc",          path: "/dashboard/kyc",    label:"KYC Verification",     icon:<svg width={15} height={15} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg> },
  { id:"support",      path: "/dashboard/support",label:"Support",              icon:<svg width={15} height={15} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> },
];
...
export default function DashboardLayout() {
  ...
  // KYC badge color
  const kycBadgeColor = kycStatus === 'verified' ? C.green : kycStatus === 'submitted' ? C.amber : kycStatus === 'rejected' ? C.red : C.muted;
  ...
  return (
    <div style={{ display:"flex", height:"100vh", background:C.bg, overflow:"hidden" }}>
      ...
      {/* Sidebar */}
      <div className={`sidebar-container ${isMobileOpen ? "open" : ""}`} style={{ width:214, background:C.surface, borderRight:`1px solid ${C.border}`,
        display:"flex", flexDirection:"column", height:"100vh", flexShrink:0 }}>
        <div style={{ padding:"20px 18px 16px", borderBottom:`1px solid ${C.border}`,
          display:"flex", alignItems:"center", gap:10 }}>
          <Mark size={32}/><div>
            <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:19, letterSpacing:2, lineHeight:1, color:C.text }}>SWIFT</div>
        ...
        <nav style={{ flex:1, padding:"0 8px", overflowY:"auto" }}>
          {NAV.map(item=>(
            <Link key={item.id} to={item.path}
              ...
              {item.soon && <span style={{ fontSize:8, padding:"2px 6px", borderRadius:4, background:C.muted2, color:C.text, letterSpacing:1 }}>SOON</span>}
            </Link>
          ))}
        </nav>
        <div style={{ padding:"8px", borderTop:`1px solid ${C.border}` }}>
          <Link to="/dashboard/settings"
            ...
            style={{ width:"100%", display:"flex", alignItems:"center", gap:10,
              ...
              color:location.pathname==="/dashboard/settings"?C.green:C.muted, fontSize:13, fontFamily:"'Outfit',sans-serif",
          ...
          <a href="/" onClick={handleLogout}
            ...
            style={{ width:"100%", display:"flex", alignItems:"center", gap:10,
              ...
              fontWeight:500, cursor:"pointer", textAlign:"left", textDecoration:"none" }}>
            Log Out
          </a>
          <div style={{ display:"flex", alignItems:"center", gap:10,
            ...
            <div style={{ width:28,height:28,borderRadius:"50%",flexShrink:0,
              ...
              fontFamily:"'Bebas Neue',sans-serif",fontSize:11,color:"#000" }}>{initials}</div>
            <div style={{ minWidth:0 }}>
              <div style={{ fontSize:11, fontWeight:600, color:C.text,
                overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{user?.full_name || 'Loading...'}</div>
              <div style={{ fontSize:9, letterSpacing:1,
                color: kycStatus === 'verified'   ? C.green
                     : kycStatus === 'submitted'  ? C.amber
                     : kycStatus === 'rejected'   ? C.red
                     : kycStatus === 'unverified' ? C.red
                     : C.muted }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0, position:"relative", overflow:"hidden" }}>
        ...
        <div className="mobile-header" style={{ height:56, display:"none", alignItems:"center", padding:"0 16px", borderBottom:`1px solid ${C.border}`, background:C.surface, flexShrink:0 }}>
            ...
            <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:20, letterSpacing:1, color:C.text }}>SWIFT TRADE</div>
        </div>
