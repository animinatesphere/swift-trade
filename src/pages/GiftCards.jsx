...
const GIFT_CARDS = [
  { id:1,  brand:"Amazon",      category:"Amazon",      color:"#FF9900", bg:"linear-gradient(135deg,#1a0800,#3d1f00)", denominations:["$25","$50","$100","$200"], rates:{"$25":34500,"$50":68000,"$100":137000,"$200":270000}, ratePerDollar:1380, country:"USA", popular:true },
  { id:2,  brand:"Amazon",      category:"Amazon",      color:"#FF9900", bg:"linear-gradient(135deg,#0d1a00,#1a3300)", denominations:["£25","£50","£100"], rates:{"£25":41000,"£50":81000,"£100":160000}, ratePerDollar:1640, country:"UK", popular:false },
  { id:3,  brand:"Steam",       category:"Steam",       color:"#66C0F4", bg:"linear-gradient(135deg,#00101a,#001f33)", denominations:["$10","$20","$50","$100"], rates:{"$10":13000,"$20":25500,"$50":63000,"$100":125000}, ratePerDollar:1300, country:"USA", popular:true },
  { id:4,  brand:"iTunes",      category:"iTunes",      color:"#FC3C44", bg:"linear-gradient(135deg,#1a0010,#330020)", denominations:["$15","$25","$50","$100"], rates:{"$15":20000,"$25":33750,"$50":67000,"$100":135000}, ratePerDollar:1350, country:"USA", popular:true },
  { id:5,  brand:"iTunes",      category:"iTunes",      color:"#FC3C44", bg:"linear-gradient(135deg,#100010,#200020)", denominations:["£15","£25","£50"], rates:{"£15":24000,"£25":39000,"£50":78000}, ratePerDollar:1560, country:"UK", popular:false },
  { id:6,  brand:"Google Play", category:"Google Play", color:"#0ECB81", bg:"linear-gradient(135deg,#001a08,#003318)", denominations:["$10","$25","$50","$100"], rates:{"$10":13700,"$25":34250,"$50":68500,"$100":137000}, ratePerDollar:1370, country:"USA", popular:false },
  { id:7,  brand:"Netflix",     category:"Netflix",     color:"#E50914", bg:"linear-gradient(135deg,#1a0000,#330000)", denominations:["$15","$30","$60","$100"], rates:{"$15":19500,"$30":39000,"$60":78000,"$100":130000}, ratePerDollar:1300, country:"USA", popular:true },
  { id:8,  brand:"Visa",        category:"Visa",        color:"#1A1F71", bg:"linear-gradient(135deg,#000818,#00102d)", denominations:["$50","$100","$200","$500"], rates:{"$50":63000,"$100":125000,"$200":248000,"$500":615000}, ratePerDollar:1260, country:"USA", popular:false },
  { id:9,  brand:"Xbox",        category:"Xbox",        color:"#107C10", bg:"linear-gradient(135deg,#001a00,#003300)", denominations:["$10","$25","$50","$100"], rates:{"$10":12800,"$25":31500,"$50":62500,"$100":124000}, ratePerDollar:1280, country:"USA", popular:false },
];
...
function SellDrawer({ card, onClose }) {
  ...
  return (
    <>
      ...
      <div className="sell-drawer" style={{
        position:"fixed", top:0, right:0, bottom:0, width:440,
        background:C.surface, borderLeft:`1px solid ${C.border}`,
        zIndex:201, overflowY:"auto", padding:"32px 32px 48px",
      }}>
        {/* Header */}
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:32 }}>
          <div>
            <div style={{ fontSize:11, color:C.muted, letterSpacing:3, marginBottom:4 }}>SELL GIFT CARD</div>
            <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:28, letterSpacing:1 }}>{card.brand} · {card.country}</div>
          </div>
          <button onClick={onClose} style={{
            background:"transparent", border:`1px solid ${C.border2}`,
            color:C.muted, width:36, height:36, borderRadius:8, fontSize:18,
          ...
          {["Amount","Card Code","Confirm"].map((s, i) => (
            <div key={s} style={{ flex:1 }}>
              <div style={{ height:3, borderRadius:2, marginBottom:6,
                background: step > i+1 ? C.green : step === i+1 ? C.green : C.border,
                opacity: step === i+1 ? 1 : step > i+1 ? 0.7 : 0.3,
              }} />
              <div style={{ fontSize:10, color: step >= i+1 ? C.green : C.muted, letterSpacing:1 }}>{s}</div>
            </div>
          ))}
        </div>
          ...
          <div style={{ animation:"fadeUp 0.3s ease" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:32 }}>
              {card.denominations.map(d => (
                <button key={d} onClick={() => setSelectedDenom(d)} style={{
                  background: selectedDenom === d ? "rgba(14,203,129,0.08)" : C.card,
                  border: selectedDenom === d ? `1px solid ${C.green}` : `1px solid ${C.border2}`,
                  borderRadius:10, padding:"16px 12px", textAlign:"center",
                  color: selectedDenom === d ? C.green : "#fff",
                  transition:"all 0.2s",
                }}>
                  <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:26, letterSpacing:1 }}>{d}</div>
                  {card.rates[d] && (
                    <div style={{ fontSize:11, color: selectedDenom === d ? C.green : C.muted, marginTop:4 }}>
                      ≈ {fmt(card.rates[d])}
                    </div>
                  )}
                </button>
            ...
            {selectedDenom && (
              <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"16px 20px", marginBottom:24 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
                  <span style={{ color:C.muted, fontSize:13 }}>Rate per USD</span>
                  <span style={{ fontFamily:"'DM Mono',monospace", fontSize:13, color:C.green }}>₦{card.ratePerDollar.toLocaleString()}</span>
                </div>
                <div style={{ display:"flex", justifyContent:"space-between" }}>
                  <span style={{ color:C.muted, fontSize:13 }}>You receive</span>
                  <span style={{ fontFamily:"'DM Mono',monospace", fontSize:15, color:"#fff", fontWeight:600 }}>{fmt(payout)}</span>
                </div>
              </div>
            )}

            <button disabled={!selectedDenom} onClick={() => setStep(2)} style={{
              width:"100%", background: selectedDenom ? C.green : C.border,
          ...
          <div style={{ animation:"fadeUp 0.3s ease" }}>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12,
              ...
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
                ...
                <span style={{ color:"#fff", fontSize:12, fontWeight:500 }}>{card.brand} {selectedDenom} · {card.country}</span>
              </div>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <span style={{ color:C.muted, fontSize:12 }}>You receive</span>
                <span style={{ fontFamily:"'DM Mono',monospace", fontSize:14, color:C.green }}>{fmt(payout)}</span>
              </div>
            </div>

            <div style={{ marginBottom:20 }}>
              <label style={{ display:"block", fontSize:12, color:C.muted, letterSpacing:2, marginBottom:10 }}>GIFT CARD CODE</label>
              <input
                value={code}
                onChange={e => setCode(e.target.value.toUpperCase())}
                placeholder="XXXX-XXXX-XXXX-XXXX"
                style={{
                  width:"100%", background:C.card, border:`1px solid ${code.length > 10 ? C.green : C.border2}`,
                  borderRadius:10, padding:"14px 16px", color:"#fff",
                  fontFamily:"'DM Mono',monospace", fontSize:15, letterSpacing:2,
                  outline:"none", transition:"border-color 0.2s",
                }}
              />
              <div style={{ fontSize:11, color:C.muted, marginTop:8, letterSpacing:0.5 }}>
                Enter the code exactly as shown on your card. Do not include spaces.
              </div>
            </div>

            <div style={{ marginBottom:28 }}>
              <label style={{ display:"block", fontSize:12, color:C.muted, letterSpacing:2, marginBottom:10 }}>CARD IMAGE (optional)</label>
              <div style={{
                border:`1px dashed ${C.border2}`, borderRadius:10, padding:"24px",
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => setStep(1)} style={{
                ...
                color:C.muted, fontWeight:500, fontSize:14, padding:"13px",
                borderRadius:10, transition:"all 0.2s",
              }}>← Back</button>
              <button disabled={code.length < 10} onClick={() => setStep(3)} style={{
                flex:2, background: code.length >= 10 ? C.green : C.border,
        ...
        {step === 3 && (
          <div style={{ animation:"fadeUp 0.3s ease" }}>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"24px", marginBottom:20 }}>
              <div style={{ fontSize:11, color:C.muted, letterSpacing:2, marginBottom:16 }}>TRADE SUMMARY</div>
              {[
                ...
                <div key={k} style={{ display:"flex", justifyContent:"space-between",
                  ...
                  <span style={{ fontSize:13, fontWeight: k==="You Receive" ? 600 : 400,
                    color: k==="You Receive" ? C.green : "#fff",
                    fontFamily: k==="You Receive" || k==="Rate" ? "'DM Mono',monospace" : "inherit" }}>
                    {v}
                  </span>
                </div>
              ))}
            ...
            <label style={{ display:"flex", gap:12, alignItems:"flex-start",
              cursor:"pointer", marginBottom:28, padding:16,
              background:"rgba(14,203,129,0.04)", border:`1px solid ${C.border}`,
              borderRadius:10 }}>
              ...
              <span style={{ fontSize:13, color:C.muted, lineHeight:1.6 }}>
                I confirm this gift card has not been used, and I agree to Swift Trade's <span style={{ color:C.green }}>Terms of Service</span>.
              </span>
            </label>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => setStep(2)} style={{
                flex:1, background:"transparent", border:`1px solid ${C.border2}`,
                color:C.muted, fontWeight:500, fontSize:14, padding:"13px", borderRadius:10,
              }}>← Back</button>
              <button disabled={!agreed} onClick={() => setStep(4)} style={{
                flex:2, background: agreed ? C.green : C.border,
        ...
        {step === 4 && (
          <div style={{ textAlign:"center", padding:"32px 0", animation:"fadeUp 0.4s ease" }}>
            ...
            <h3 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:32, letterSpacing:1, marginBottom:12 }}>
              TRADE SUBMITTED
            </h3>
            ...
            <p style={{ color:C.muted, fontSize:14, lineHeight:1.7, marginBottom:32 }}>
              You'll receive <span style={{ color:C.green, fontWeight:600 }}>{fmt(payout)}</span> in your Swift Trade NGN wallet within 5–15 minutes.
            </p>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12,
              padding:"16px 20px", marginBottom:32, textAlign:"left" }}>
              <div style={{ fontSize:11, color:C.muted, letterSpacing:2, marginBottom:12 }}>TRANSACTION ID</div>
              <div style={{ fontFamily:"'DM Mono',monospace", fontSize:13, color:C.green, wordBreak:"break-all" }}>
                ST-{Date.now().toString(36).toUpperCase()}
              </div>
            </div>
            <button onClick={onClose} style={{
              width:"100%", background:C.green, color:"#000", fontWeight:700,
              fontSize:15, padding:"14px", borderRadius:10, border:"none",
            }}>Done</button>
          </div>
        )}
      </div>
...
function GiftCardItem({ card, onSell, delay = 0 }) {
  return (
    <FadeIn delay={delay}>
      <div className="gc-sell-card" style={{
        ...
        <div style={{
          background:card.bg, padding:"28px 24px 24px",
          position:"relative", overflow:"hidden",
        }}>
          <div className="gc-shine" style={{
          ...
          {card.popular && (
            <span style={{
              ...
              color:C.green, fontSize:9, letterSpacing:2, padding:"4px 8px",
              borderRadius:100, fontWeight:600,
            }}>HOT</span>
          )}
          <div style={{ fontSize:11, fontWeight:600, letterSpacing:3,
            color:card.color, opacity:0.8, marginBottom:16, textTransform:"uppercase" }}>
            {card.brand}
          </div>
          <div style={{ display:"flex", alignItems:"flex-end", gap:8, marginBottom:6 }}>
            <span style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:36, letterSpacing:1, lineHeight:1 }}>
              {card.denominations[0]}
            </span>
            <span style={{ color:C.muted, fontSize:11, marginBottom:4 }}>– {card.denominations[card.denominations.length-1]}</span>
          </div>
          <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)" }}>{card.country} · Gift Card</div>
        </div>

        <div style={{ padding:"20px 24px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
            <div>
              <div style={{ fontSize:11, color:C.muted, letterSpacing:1, marginBottom:4 }}>RATE / USD</div>
              <div style={{ fontFamily:"'DM Mono',monospace", fontSize:18, color:C.green, fontWeight:500 }}>
                ₦{card.ratePerDollar.toLocaleString()}
              </div>
            </div>
            <div style={{ textAlign:"right" }}>
              <div style={{ fontSize:11, color:C.muted, letterSpacing:1, marginBottom:4 }}>DENOMINATIONS</div>
              <div style={{ fontSize:12, color:"#aaa" }}>{card.denominations.join(", ")}</div>
            </div>
          </div>

          <button className="sell-btn-inner" onClick={() => onSell(card)} style={{
...
export default function GiftCardsPage() {
  ...
  const filtered = GIFT_CARDS.filter(c => {
    ...
    const matchSearch = c.brand.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", color: C.text, fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div className="hero-padding" style={{
        ...
        <div style={{
          position:"absolute", top:50, left:200, width:400, height:400,
        ...
        <div className="hero-grid" style={{ 
          ...
          <div className="hero-text">
            ...
            <h1 style={{ fontFamily:"'Bebas Neue',sans-serif",
              fontSize:"clamp(52px,7vw,96px)", lineHeight:0.9,
              letterSpacing:2, marginBottom:24 }}>
              SELL YOUR<br />
              <span style={{ color:C.amber }}>GIFT CARDS</span><br />
              FOR NAIRA
            </h1>
            <p style={{ color:C.muted, fontSize:17, lineHeight:1.7, fontWeight:300, maxWidth:500, marginBottom: 40 }}>
              Trade Amazon, Steam, iTunes, Netflix and more — get instant NGN in your wallet. No stress, no waiting.
            </p>
            ...
            <div className="hero-stats" style={{ display:"flex", gap:48 }}>
              {[
                ...
                <div key={s.label}>
                  <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:28, color:C.green, letterSpacing:1 }}>{s.val}</div>
                  <div style={{ fontSize:12, color:C.muted, letterSpacing:1 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="hero-image" style={{ position: "relative", animation: "fadeUp 1s ease" }}>
            ...
            <img src={heroTrader} alt="Professional Trader" style={{
              width: "100%", height: "auto", borderRadius: 24, 
              border: `1px solid ${C.border2}`, boxShadow: "0 40px 80px rgba(0,0,0,0.6)",
              position: "relative", zIndex: 1, objectFit: "cover", aspectRatio: "4/5"
            }} />
          </div>
        </div>
      </div>
        ...
        <div className="filters-scroll" style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {CATEGORIES.map(cat => (
            <button key={cat} className="category-pill" onClick={() => setActiveCategory(cat)} style={{
              ...
              border: activeCategory === cat ? `1px solid ${C.green}` : `1px solid ${C.border2}`,
              color: activeCategory === cat ? C.green : C.muted,
              fontSize:13, padding:"7px 16px", borderRadius:100,
              fontFamily:"'Outfit',sans-serif", fontWeight:500, transition:"all 0.2s",
            }}>{cat}</button>
          ))}
        </div>
        ...
        <div style={{ position:"relative" }}>
          ...
          <input className="search-input" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            ...
            style={{
              ...
              padding:"9px 16px 9px 38px", color:"#fff", fontSize:14,
              fontFamily:"'Outfit',sans-serif", outline:"none", width:200,
            }} />
        </div>
      </div>
      ...
      <div className="section-padding" style={{ padding:"48px 64px" }}>
        {filtered.length === 0 ? (
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:20 }}>
            {filtered.map((card, i) => (
              <GiftCardItem key={card.id} card={card} onSell={setSellingCard} delay={i * 0.05} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="section-padding" style={{ padding:"100px 64px", borderTop:`1px solid ${C.border}`, background:C.surface }}>
        <div className="how-it-works-grid" style={{ 
          ...
          <FadeIn>
            <div style={{ position: "relative" }}>
              <div style={{
                position: "absolute", inset: -10, border: `1px solid ${C.green}33`,
                borderRadius: 24, transform: "rotate(-3deg)", zIndex: 0
              }} />
              <img src={lifestyleGiftcard} alt="Gift card trading" style={{
              ...
              <div style={{
                ...
                background: C.green, color: "#000", padding: "12px 24px",
                borderRadius: 12, fontWeight: 700, fontSize: 13, zIndex: 2,
          ...
          <div>
            <div style={{ marginBottom:48 }}>
              <div style={{ display:"inline-flex", alignItems:"center", gap:10, color:C.green,
                fontSize:11, letterSpacing:4, textTransform:"uppercase", fontWeight:500, marginBottom:12 }}>
                <span style={{ display:"block", width:24, height:1, background:C.green }} />
                How It Works
              </div>
              <h2 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:48, letterSpacing:1, lineHeight:1 }}>
                ...
                <FadeIn key={s.n} delay={i * 0.1}>
                  <div className="step-card" style={{
                    ...
                    <div className="step-icon-box" style={{
                      ...
                      display:"flex", alignItems:"center", justifyContent:"center",
                      background:"rgba(14,203,129,0.06)", border:`1px solid ${C.border2}`,
                      transition:"all 0.3s", flexShrink: 0
                    }}>{s.icon}</div>
                    <div>
                      <h4 style={{ fontSize:18, fontWeight:600, marginBottom:4 }}>{s.title}</h4>
